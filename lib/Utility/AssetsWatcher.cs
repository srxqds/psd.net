...
public class AssetsWatcher : AssetPostprocessor
{
    ...
    {
        if (assetPath == null || string.IsNullOrEmpty(filterExtension)) return false;
        string fileName = Path.GetFileNameWithoutExtension(assetPath);
        if(!string.IsNullOrEmpty(searchFilter) && !fileName.ToLower().Contains(searchFilter.ToLower()))
            return false;
        return assetPath.EndsWith(filterExtension);
    }
    ...
    public static List<string> Refresh(string fileExt,string search)
    {
        filterExtension = fileExt;
        searchFilter = search;
        Refresh();
